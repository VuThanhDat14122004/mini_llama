...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        # todo
        '''
        Tính theo chiều cuối cùng vì chiều đầu tiên sẽ là batch size keep dim để giữ số chiều sau khi tính toán
        '''
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        d_k = torch.tensor(key.shape[-1])
        numerator = torch.matmul(query, key.permute(0,1,3,2))
        in_softmax = numerator/torch.sqrt(d_k)
        return torch.matmul(torch.softmax(in_softmax, dim=-1), value)
    ...
    def forward(
        ...
        # shape: batchsize x seqlen x config.dim
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        normalize_x = self.attention_norm(x)
        ## attention
        output_attention = self.attention(normalize_x)
        ## residual attention
        residual_attention = output_attention + x
        ## normalize before feed forward
        ffnorm = self.ffn_norm(residual_attention)
        ## feed forward
        output_feedforward = self.feed_forward(ffnorm)
        ## residual
        return output_feedforward + residual_attention
...
class Llama(LlamaPreTrainedModel):
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        for layer in self.layers:
            h = layer(h)
        h = self.norm(h) # shape: batchsize x seqlen x config.dim
        if targets is not None:
            # if we are given some desired targets also calculate the loss
            # đưa toàn bộ h vào linear layer để lấy logits vì đây là training mode, cần logist của toàn bộ câu để tính loss
            logits = self.output(h) # shape: batchsize x seqlen x vocab_size
        else:
            # inference-time mini-optimization: only forward the output on the very last position
            # trong inference mode chỉ cần đưa h cuối cùng vào linear layer để lấy logits (tất cả h đã được đi qua các layer attention và feedforward nên đã có thông tin của cả câu)
            logits = self.output(h[:, [-1], :]) # note: using list [-1] to preserve the time dim
            # shape: batchsize x 1 x vocab_size
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        """
        idx: LongTensor of shape (b,t) where b is the batch size and t is the length of the context. idx is the input sequence,
        each elemnet in idx is an index into the vocabulary.
        max_new_tokens: int, the maximum number of tokens to generate after idx
        """
        ...
        for _ in range(max_new_tokens):
            ...
            logits, _ = self(idx_cond) # gọi hàm forward # shape: batchsize x 1 x vocab_size
            logits = logits[:, -1, :] # crop to just the final time step
            # shape: batchsize x vocab_size
            # todo
            idx_next = None
            if temperature == 0.0:
                ...
                idx_next = torch.multinomial(probs, num_samples=1) # lấy mẫu từ một phân phối xác suất rời rạc probs
